...
// MARK: - Anchored banner

/// A persistent anchored-adaptive banner for free users — shows on every screen
/// it's placed on, regardless of how much content there is (unlike native slots,
/// which need a long feed). Renders nothing for Plus members or until the SDK is
/// ready. Place it above the tab bar.
struct BottomBannerAd: View {
    ...
    var body: some View {
        Group {
            if !subscriptions.isPlus, ads.isReady, let unit = AdConfig.bannerUnitID {
                let width = UIScreen.main.bounds.width
                let size = currentOrientationAnchoredAdaptiveBanner(width: width)
                BannerAdContainer(adUnitID: unit, adSize: size)
                    .frame(width: width, height: size.size.height)
            }
        }
        .task { if !subscriptions.isPlus { ads.startIfNeeded() } }
    }
}

/// Wraps `BannerView` (UIKit) for SwiftUI.
