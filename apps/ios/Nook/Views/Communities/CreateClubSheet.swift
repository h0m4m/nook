    ...
    var body: some View {
        ...
        .fullScreenCover(isPresented: $showBannerCrop) {
            if let rawBannerImage {
                ClubCropView(image: rawBannerImage, cropAspect: bannerCropAspect) { cropped in
        ...
        .fullScreenCover(isPresented: $showIconCrop) {
            if let rawIconImage {
                ClubCropView(image: rawIconImage, cropAspect: iconCropAspect) { cropped in
                    withAnimation(.easeOut(duration: 0.2)) {
                        iconImage = cropped
                    }
                }
...
private struct ClubCropView: View {
    let image: UIImage
    ...
    private var cropCornerRadius: CGFloat { cropAspect == 1.0 ? 22 : 24 }
...
private extension View {
    func clubReverseMask<Mask: View>(@ViewBuilder _ mask: () -> Mask) -> some View {
        self.mask(
            ZStack {
                Rectangle()
                mask()
